 ...
 */
/**
 * @brief  Called from the capture thread when the AFE reports a wake word
 *
 * @note   Keep it short - it runs on the audio path. Signal a task and return.
 */
typedef void (*esp_capture_wake_word_cb)(int wake_word_index, void *ctx);

typedef struct {
    esp_capture_wake_word_cb wake_cb;     /*!< Optional wake-word callback. The AFE always computes
                                               `wakeup_state`; upstream discards it, this fork
                                               forwards it. Requires a WakeNet model in the
                                               "model" partition. */
    void       *wake_ctx;                 /*!< Context passed to `wake_cb` */
