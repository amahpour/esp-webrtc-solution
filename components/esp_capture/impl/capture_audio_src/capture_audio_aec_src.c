...
typedef struct {
    ...
    esp_capture_wake_word_cb    wake_cb;
    void                       *wake_ctx;
...
static esp_capture_err_t audio_aec_src_read_frame(esp_capture_audio_src_if_t *h, esp_capture_stream_frame_t *frame)
{
    ...
    while (need_size > 0) {
        ...
        if (use_silent == false) {
            ...
            if (res->wakeup_state == WAKENET_DETECTED && src->wake_cb) {
                ESP_LOGI(TAG, "Wake word %d detected", res->wake_word_index);
                src->wake_cb(res->wake_word_index, src->wake_ctx);
            }
